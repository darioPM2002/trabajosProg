...
class Ordenamiento {
public:
    using FuncionOrdenamiento = std::function<std::vector<T>(std::vector<T>, bool(*)(T, T))>;
    ...
    std::vector<T> ordenarPorMetodo(std::vector<T> e, FuncionOrdenamiento , bool (*comparar)(T, T));
    static bool asc(T,T);
...
template <typename T>
std::vector<T> Ordenamiento<T>::ordenarPorMetodo(std::vector<T> e,  FuncionOrdenamiento metodoOrdenamiento, bool(*comparar)(T, T)) {
    // Ordenar el vector utilizando el método de ordenamiento proporcionado
    e = metodoOrdenamiento(e, comparar);
